...
public class StatAdminService implements StatSideAdminInterfaceRemote{
	...
	public List<Object[]> CountNbClientByType() {
		// TODO Auto-generated method stub
		 List<Object[]> results = em.createQuery("SELECT DISTINCT p.clientType,count(*) from Person p group by p.clientType", Object[].class).getResultList();
		 return results;
	}
	...
	public List<Object[]> BeneficeByProject() {
		// TODO Auto-generated method stub
		List<Object[]> results = em.createQuery("select ROUND(sum( ( m.montant/DATEDIFF(m.mandateId.dateFin,m.mandateId.dateDebut) -(m.ressource.salary/30) )*DATEDIFF(m.mandateId.dateFin,m.mandateId.dateDebut) ),2),m.projet.id from Mandate m group by m.projet.id", Object[].class).getResultList();
		return results;
	}
	...
	public List<Object[]> BeneficeByClient() {
		// TODO Auto-generated method stub
		List<Object[]> results = em.createQuery("select ROUND(sum( ( m.montant/DATEDIFF(m.mandateId.dateFin,m.mandateId.dateDebut) -(m.ressource.salary/30) )*DATEDIFF(m.mandateId.dateFin,m.mandateId.dateDebut) ),2),m.projet.client.id from Mandate m group by m.projet.client.id", Object[].class).getResultList();
		return results;	}
